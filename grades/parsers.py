...
:Classes:
    - `RSTTableParser`: Parse fully-formed tables represented with a grid.
...
def _is_eval_name(name):
    """Determine if name is an evaluation name."""
    if name.upper().startswith(('TEST', 'EXAM', 'MIDTERM',
                                'QUIZ', 'FINAL', 'EVAL')):
        return True
    return False
    ...
    def find_head_body_sep(self):
        ...
        if self.head_body_sep == 0 or self.head_body_sep == (len(self.block)
            ...
            raise TableMarkupError('The head/body row separator may not be '
                                   ...
                                   offset=i)
        if self.head_body_sep is None:
            raise TableMarkupError('No header rows; there must be three header rows')


class RSTTableParser(TableParser):
    ...
    def structure_from_cells(self):
        ...
        # Convert the structure to a GradesTable.
        table = grades.GradesTable()
        for i, cell in enumerate(headrows[0]):
            name = ' '.join(cell[3])
            if _is_eval_name(name):
                evalu = {'max_grade': _to_float(headrows[1][i][3][0], 100.),
                         'weight': _to_float(headrows[2][i][3][0], 0.)}
        ...
        for row in bodyrows:
            ...
            for i, entry in enumerate(row):
                if i >= len(table.columns) or entry[3][0].startswith('/'):
                    break
